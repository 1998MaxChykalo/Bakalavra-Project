...
export const GraphPageComponent = ({ adjacencyMatrix, users }) => {
  ...
  const r = scaleLinear().range([1, 6]).domain(extent(verticesWeight));
  // const width = scaleLinear().range([1, 4]).domain(extent([-9, 9]));
  // const linkLength = scaleLinear().range([40, 10]).domain(extent([-9, 9]));
  const interpolateNodeToColor = scaleLinear().domain(extent(verticesWeight)).range(['red', 'green']);
  const interpolateWeightToColor = scaleLinear().domain([-9, 9]).range(['red', 'green']);
  return (
    <div>
      <ForceGraph2D
        ...
        linkDirectionalParticleSpeed={link => Math.abs(link.weight) * 0.0004}
        linkDirectionalParticleWidth={1.5}
        linkDirectionalArrowLength={1}
        linkDirectionalArrowColor={link => interpolateWeightToColor(link.weight)}
        linkDirectionalArrowRelPos={1}
        // linkCurvature={0.5}
        linkDirectionalParticleColor={link => interpolateWeightToColor(link.weight)}
      />
    </div>
  );
}
